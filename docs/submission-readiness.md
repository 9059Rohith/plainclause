...
**Repository and local demonstration: ready for technical review. Durable hosted submission: pending.** All checked local engineering gates are recorded as passed in the [test report](test-report-2026-09-25.md), subject to the scope of those tests. The previously requested requirement for a durable live cloud link is open: Railway rejects source uploads because its workspace is restricted. Therefore a claim that *all* submission parameters or the whole project are "100% submission ready" would be inaccurate.
...
Automated checks ran on the documented machine and configuration. They establish specific observed behavior, not full correctness for every input or environment. The performance report is a small synthetic local measurement, not a service-level objective. Security controls reduce risk but have not undergone penetration testing. The live preview is a test tunnel and does not satisfy durable hosting.
